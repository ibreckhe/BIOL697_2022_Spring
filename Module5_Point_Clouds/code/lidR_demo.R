...
##Sets up workspace
##install.packages(c("lidR","RCSF","raster","rgdal","sf","future"))
library(lidR)
library(RCSF)
library(raster)
library(rgdal)
library(sf)
library(future)
setwd("~/code/BIOL697_2022_Spring/")

####Basic processing demo.

##Loads point clouds
lidar_tile <- readLAS("./Module5_Point_Clouds/data/LiDAR_tiles/Gothic_LiDAR_pointcloud_2019_tile1.laz")
lidar_tile
sfm_tile <- readLAS("./Module5_Point_Clouds/data/SfM_tiles/Gothic_SfM_pointcloud_2019_tile1.laz")
sfm_tile
...
writeRaster(sfm_dem,"./Module5_Point_Clouds/data/output_DEM.tif",
            overwrite=TRUE)
...
crown_metrics <- delineate_crowns(sfm_seg, 
                                   func = ~custom_tree_metrics(z = Z, 
                                                               gcc = GCC))
spplot(crown_metrics, "GCC_mean", col.regions = hcl.colors(30)) 
st_write(st_as_sf(crown_metrics),dsn="./Module5_Point_Clouds/data/crown_polygons.gpkg")

####Tiled processing demo (for clouds that can't fit in memory. #####

lidar_catalog <- readLAScatalog("./Module5_Point_Clouds/data/LiDAR_tiles/")
...
opt_output_files(lidar_catalog) <- "./Module5_Point_Clouds/data/raster_tiles/chunk_{ID}"
